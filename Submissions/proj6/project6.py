# -*- coding: utf-8 -*-
"""Copy of Project6.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1rUpzhn_lyusPbPvRtMVaKKVEUq40dBXM
...
def generate_random_point(target_node: gtsam.Point3) -> gtsam.Point3:
  ...
  goalprob = np.random.randint(10)
  if goalprob < 2:
    return target_node
  random_node = rng.uniform(0, 10, size=(3,))
...
def distance_euclidean(point_a: gtsam.Point3, point_b: gtsam.Point3) -> float:
  ...
  pointsa = [point_a[0], point_a[1], point_a[2]]
  pointsb = [point_b[0], point_b[1], point_b[2]]
  distance_euclidean_m = np.linalg.norm(np.array(pointsb) - np.array(pointsa))
  ######## End student code  ########
...
def find_nearest_node(rrt: List[gtsam.Point3], new_node: gtsam.Point3):
  ...
  distances = np.linalg.norm(np.array(rrt) - new_node, axis=1)
...
def steer_naive(parent_node: gtsam.Point3, target_node: gtsam.Point3, fraction = 0.2):
  ...
  vec = np.array([target_node[i] for i in range(3)]) - np.array([parent_node[j] for j in range(3)])
  length = np.linalg.norm(vec)
  steer_node = parent_node + (vec * fraction)
...
Here's a method to do this. We've already implemented it for you 😉
"""
...
def compute_attitude_from_ypr(yaw_rad: float, pitch_rad: float, roll_rad: float) -> gtsam.Rot3:
  ...
  attitude_mat33 = gtsam.Rot3.Yaw(yaw_rad) * gtsam.Rot3.Pitch(pitch_rad) * gtsam.Rot3.Roll(roll_rad)
...
def compute_force(attitude_mat33: gtsam.Rot3, thrust_newton: float) -> gtsam.Point3:
  ...
  thrustvec = np.array([0, 0, thrust_newton])
  force_vector_newton = gtsam.Point3(np.dot(attitude_mat33.matrix(), thrustvec))
...
def compute_terminal_velocity(force_vector_newton: gtsam.Point3, kd: float = 0.0425) -> gtsam.Point3:
  ...
  direction = [-1 if e < 0 else 1 for e in force_vector_newton]
  terminal_velocity_ms = gtsam.Point3(np.sqrt(abs(force_vector_newton / kd)) * direction)
...
def generate_random_pose(target_pose_node: gtsam.Pose3) -> gtsam.Pose3:
  ...
  randpos = rng.uniform(0, 10, size=(3,))
  randangles = np.radians(rng.uniform(-60, 60, size=(3,)))
  attitude = compute_attitude_from_ypr(randangles[0], randangles[1], randangles[2])
  random_pose_node = gtsam.Pose3(r=attitude, t=randpos)
...
def find_nearest_pose(rrt: List[gtsam.Pose3], new_pose_node: gtsam.Pose3):
  ...
  for i in range(len(rrt)):
    bd = helpers.distance_between_poses(rrt[i], new_pose_node)
    if bd < mindist:
      nearest_pose = rrt[i]
...
def steer_with_terminal_velocity(current_pose_node: gtsam.Pose3, target_pose_node: gtsam.Pose3, duration_s: float = 0.1) -> gtsam.Pose3:
  ...
  vec = target_pose_node.translation() - current_pose_node.translation()
  length = np.linalg.norm(vec)
  unit = vec / length
  newatt = helpers.get_new_attitude(current_pose_node, unit)
  tv = compute_terminal_velocity(compute_force(newatt, 20.0))
  steer_pose_node = gtsam.Pose3(r=newatt, t=current_pose_node.translation() + (tv * duration_s))
...
def compute_force_with_gravity(attitude_mat33: gtsam.Rot3, thrust_newton: float, mass_kg: float = 1.0) -> gtsam.Point3:
  ...
  thrust = np.array([0, 0, thrust_newton])
  gravity = np.array([0, 0, 10])
  force_newton = gtsam.Point3(np.dot(attitude_mat33.matrix(), thrust) - gravity)
    ...
    for p in pitch_values_deg:
      for r in roll_values_deg:
        for t in thrust_values_newton:
          att = current_pose_node.rotation() * compute_attitude_from_ypr(yaw_rad=np.radians(y), pitch_rad=np.radians(p), roll_rad=np.radians(r))
          force = compute_force_with_gravity(att, t)
          vel = compute_terminal_velocity(force)
          pose = gtsam.Pose3(r=att, t=current_pose_node.translation() + (vel * duration_s))
          if helpers.distance_between_poses(pose, target_pose_node) < mindist:
            mindist = helpers.distance_between_poses(pose, target_pose_node)
            steer_pose_node = pose
